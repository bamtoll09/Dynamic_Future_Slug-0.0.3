...
namespace RGEngine
{
		Input::Input(HWND hWnd) : m_posMouse()
		{
			// Initialize the location of the mouse on the screen.
			m_posMouse.x = m_posMouse.y = 0;
		...
		void Input::Update(void)
		{
			...
			m_posMouse.x += (int)m_stateMouse.lX;
			m_posMouse.y += (int)m_stateMouse.lY;
		}
...
#undef RKEY
		...
		void Input::GetMouse(Types::Point &mouse) const
		{
			mouse = m_posMouse;
		}
		void Input::GetMouse(Types::Point *mouse) const
		...
		void Input::GetMouse(Math::Vector &mouse) const
		{
			mouse.SetVector((float)m_posMouse.x, (float)m_posMouse.y);
		}
		void Input::GetMouse(Math::Vector *mouse) const
		{
			mouse->SetVector((float)m_posMouse.x, (float)m_posMouse.y);
